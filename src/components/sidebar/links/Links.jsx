...
const Links = () => {
  ...
  return (
    <motion.div className="links" variants={variants}>{items.map((item)=>{ if(item === "Resume"){ return (
      <motion.a href={`https://drive.google.com/file/d/1v0lIzpSLelZ7Uvuo1u1uNyITGuioPTPg/view?usp=sharing`} target = "_blank" key={item} variants = {itemvariants} whileHover={{scale:1.1}} whileTap={{scale:0.95}}>{item } < FaExternalLinkAlt />
</motion.a>
